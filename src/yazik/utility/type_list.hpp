...
// from https://github.com/actor-framework/actor-framework/blob/master/libcaf_core/caf/detail/type_list.hpp

#include <yazik/utility/unit.hpp>
#include <yazik/utility/none.hpp>
...
namespace yazik::utility {
    ...
    template <class List, template <class...> class Fun>
    struct tl_reduce;

    template <typename... Ts, template <class...> class Fun>
    struct tl_reduce<type_list<Ts...>, Fun> {
        using type = typename Fun<Ts...>::type;
    };

    template <class List, template <class...> class Fun>
    using tl_reduce_t = typename tl_reduce<List, Fun>::type;
